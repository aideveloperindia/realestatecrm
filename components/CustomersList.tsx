...
export default function CustomersList() {
  ...
  const handleSendCustomerReminder = async (customer: Customer) => {
    if (!customer.opt_in_whatsapp) {
      if (!confirm('Customer has not opted in for WhatsApp. Send reminder anyway?')) {
        return;
      }
    }
    ...
    try {
      const typesText =
        customer.preferred_types.length > 0
          ? customer.preferred_types.join(', ')
          : 'the property types you prefer';

      const messageText = `Hi ${customer.name}, this is a reminder from Real Estate Sales CRM. We have property options matching your preferred types: ${typesText}. Would you like details and a visit? Reply YES.`;
      ...
      console.error('Error sending customer WhatsApp reminder:', error);
              ...
              <div className="px-4 py-4 sm:px-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 mt-3 sm:mt-0">
                    ...
                    <button
                      type="button"
                      onClick={() => handleSendCustomerReminder(customer)}
                    ...
                    >
                      {sendingCustomerId === customer._id ? 'Opening...' : 'WhatsApp Reminder'}
                    </button>
                    <Link
                      href={`/customers/${customer._id}`}
